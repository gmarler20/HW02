...
/** Remove an entry
	...
	*/
string Phone_Directory::remove_entry(const string& name) // Exercise 1.7: Iain and Griffen worked together.
{
	int name_index = find(name);
 
	;
	if (name_index != -1) {
		return the_directory[name_index].get_number();
		for (int i = name_index; i < size - 1; i++) {
			the_directory[i] = the_directory[i + 1];
			
			/*Directory_Entry* new_directory = new Directory_Entry[capacity]; // Also attempted to do something similar to reallocate to overwrite the array, but was not successful.
